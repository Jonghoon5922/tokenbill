...
def list_providers(user: models.User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    out = []
    for p in PROVIDERS:
        key = next((k for k in user.keys if k.provider == p), None)
        month_cost = 0.0
        if key:
            month_cost = db.query(func.coalesce(func.sum(models.UsageDaily.cost_usd), 0.0)).filter(
                models.UsageDaily.user_id == user.id,
                models.UsageDaily.provider == p,
        ...
        out.append({
            ...
            "status": key.last_status if key else None,
...
def add_provider(body: KeyIn, user: models.User = Depends(current_user), db: Session = Depends(get_db)):
    existing = next((k for k in user.keys if k.provider == body.provider), None)
    if existing:
        db.delete(existing)
        db.flush()
...
def delete_provider(provider: str, user: models.User = Depends(current_user), db: Session = Depends(get_db)):
    key = next((k for k in user.keys if k.provider == provider), None)
    if not key:
        raise HTTPException(404, "연결된 키가 없습니다")
    db.delete(key)
    db.query(models.UsageDaily).filter_by(user_id=user.id, provider=provider).delete()
    db.commit()
    return {"ok": True}
